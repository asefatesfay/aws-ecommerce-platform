...
fully-configured FastAPI application with:
  ...
  - CORS middleware
  - /health endpoint that verifies Aurora + Redis connectivity (Requirement 14.6)
  - Graceful shutdown hooks

Usage in a service's own main.py::

    from shared.app.main import create_app
...
from shared.app.cache import close_cache, get_redis, init_cache
from shared.app.db import close_db, get_engine, init_db
...
async def lifespan(app: FastAPI):  # type: ignore[type-arg]
    ...
    # Verify connectivity so the container fails fast on misconfiguration (Req 14.6)
    await _check_db()
    await _check_redis()
...
async def health_check() -> JSONResponse:
    """
    Readiness probe — Requirement 14.6.
...
def create_app(
    *,
    ...
    # Health endpoint — no auth required (Requirement 14.6)
